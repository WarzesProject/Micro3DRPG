...
#include <Core/Time/TimeManager.h>
	...
	#include <Core/RenderDocGraphicsDebugger.h>
#endif
#ifdef RENDERER_RUNTIME_IMGUI
	#include <DebugGui/ImGuiLog.h>
...
#include <Context.h>
#include "Engine/Core/File/IFileManager.h"
#include "Engine/Core/File/IFile.h"
#include "Engine/Resource/CompositorNode/CompositorNodeInstance.h"
...
//[ Global definitions                                    ]
//[-------------------------------------------------------]
static char const*    VIRTUAL_SETTINGS_FILENAME	= "LocalData/FirstSceneExample.ini";
...
//				rendererRuntime.getDebugGuiManager().openMetricsWindow();
	...
	rendererRuntime.getSceneResourceManager().loadSceneResourceByAssetId(SCENE_ASSET_ID, mSceneResourceId, this);

	// Load the material resource we're going to clone
	rendererRuntime.getMaterialResourceManager().loadMaterialResourceByAssetId(IMROD_MATERIAL_ASSET_ID, mMaterialResourceId, this);
...
void FirstScene::onLoadingStateChange(const RendererRuntime::IResource& resource)
{
	...
	if (resource.getAssetId() == SCENE_ASSET_ID)
	{
	...
	else if (RendererRuntime::IResource::LoadingState::LOADED == loadingState && resource.getAssetId() == IMROD_MATERIAL_ASSET_ID)
	{
...
void FirstScene::loadIni()
{
	...
	if (fileManager.doesFileExist(VIRTUAL_SETTINGS_FILENAME))
	{
		RendererRuntime::IFile* file = fileManager.openFile(RendererRuntime::IFileManager::FileMode::READ, VIRTUAL_SETTINGS_FILENAME);
		if (nullptr != file)
		{
...
void FirstScene::saveIni()
{
	if (nullptr != mIni)
	{
		...
		RendererRuntime::IFile* file = fileManager.openFile(RendererRuntime::IFileManager::FileMode::WRITE, VIRTUAL_SETTINGS_FILENAME);
		if (nullptr != file)
		{
			RendererRuntime::IRendererRuntime& rendererRuntime = getRendererRuntimeSafe();
			char temp[256];
