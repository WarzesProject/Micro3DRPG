...
//-----------------------------------------------------------------------------
bool WindowSDL::Init(const char *windowTitle)
{
	strncpy(m_windowTitle, windowTitle, 64);

	if (SDL_Init(0) == 0)
	{
		m_sdlWindow = SDL_CreateWindow(m_windowTitle, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1024, 768, SDL_WINDOW_HIDDEN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
	}
	else
		return false;
...
//-----------------------------------------------------------------------------
bool WindowSDL::ProcessMessages()
{
...
//-----------------------------------------------------------------------------
void WindowSDL::GetWindowSize(int &width, int &height) const
{
	if (nullptr != m_sdlWindow)
...
//-----------------------------------------------------------------------------
handle WindowSDL::GetNativeWindowHandle() const
{
...
#ifdef RENDERER_RUNTIME_IMGUI
void WindowSDL::initializeImGuiKeyMap()
{
...
//-----------------------------------------------------------------------------
void WindowSDL::processImGuiSdl2Event(const SDL_Event& sdlEvent)
{
...
//-----------------------------------------------------------------------------
void WindowSDL::updateImGuiMousePositionAndButtons()
{
	assert(ImGui::GetCurrentContext() != nullptr);
	ImGuiIO& imGuiIo = ImGui::GetIO();
