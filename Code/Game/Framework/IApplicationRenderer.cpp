...
Renderer::IRenderer* IApplicationRenderer::createRendererInstance(const char* rendererName)
{
	...
	if (nullptr != rendererName)
	{
		...
		Renderer::ILog& log = (nullptr != mExampleBase.getCustomLog()) ? *mExampleBase.getCustomLog() : ::g_DefaultLog;
		#ifdef _WIN32
			mRendererContext = new Renderer::Context(log, ::g_DefaultAssert, g_DefaultAllocator, getNativeWindowHandle());
		#elif LINUX
