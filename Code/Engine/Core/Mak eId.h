...
private:
	...
	uint32_t m_Count;    // Number of ranges in list
	uint32_t m_Capacity; // Total capacity of range list

	MakeID & operator=(const MakeID &) = delete;
	MakeID(const MakeID &) = delete;

public:
	explicit MakeID(const uint32_t max_id = std::numeric_limits<uint32_t>::max())
	{
	...
	bool CreateRangeID(uint32_t &id, const uint32_t count)
	{
		...
		do
		{
			const uint32_t range_count = 1u + m_Ranges[i].m_Last - m_Ranges[i].m_First;
	...
	bool DestroyID(const uint32_t id)
	{
	...
	{
		const uint32_t end_id = static_cast<uint32_t>(id + count);

		// Binary search of the range list
		uint32_t i0 = 0u;
		uint32_t i1 = m_Count - 1u;

		for (;;)
		{
			const uint32_t i = (i0 + i1) / 2u;
	...
	bool IsID(const uint32_t id) const
	{
		// Binary search of the range list
		uint32_t i0 = 0u;
		uint32_t i1 = m_Count - 1u;
	...
	uint32_t GetAvailableIDs() const
	{
	...
	uint32_t GetLargestContinuousRange() const
	{
		...
		do
		{
			uint32_t count = m_Ranges[i].m_Last - m_Ranges[i].m_First + 1u;
			if (count > max_count)
				max_count = count;
